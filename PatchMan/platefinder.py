...
class PlateFinder(object):
    ...
    def checkBlob(self, img, blob):
        ...
        (x, y), w, h = (
            blob.topLeftCorner(), blob.minRectWidth(), blob.minRectHeight()
        )
        margin = 20
    ...
    def findSimbols(self, srcimg):
        img = srcimg.crop(
            3, 3, srcimg.width - 6, srcimg.height - 6
        ).resize(h=50).dilate()
    ...
    def findChars(self, img):
        # busca 6 o mas blobs dentro de la patente y evalua sus caracteres uno
        # por uno los genera y luego findSimbols los lista y los agrupa en un
        # string
    ...
    def prepare(self, img, scale=True):
        #89/94
        img = img / 3 if(scale) else img

        # return (img - img.binarize().morphOpen()).gaussianBlur().binarize()
        # return img.grayscale().gaussianBlur(
        #     window=(5, 5), grayscale=True
        # ).sobel(1, 0, True,3).binarize(10)
